...
for cell = 1:MN
    if(cell == (targetCell(2)-1)*M +targetCell(1)) %if cell is target
        ...
        controls = availableControls(cell,wallsMatrix,M,controlSpace);
        for i = 1:size(controls,2)
            ...
            %STAY
            P(cell,cellArrWithCont,controls(i)) = disturbanceSpace(intersect(find(disturbanceSpace(:,1) == 0),find(disturbanceSpace(:,2) == 0)),3);
            %LEFT
            if(wallsCellArrival(3) == 0)
                P(cell,cellArrWithCont - M,controls(i)) = disturbanceSpace(find(disturbanceSpace(:,1) == -1),3);
            else
                P(cell,cellArrWithCont,controls(i)) = P(cell,cellArrWithCont,controls(i)) + disturbanceSpace(find(disturbanceSpace(:,1) == -1),3);
            end
            %BOTTOM
            if(wallsCellArrival(4) == 0)
                P(cell,cellArrWithCont - 1,controls(i)) = disturbanceSpace(find(disturbanceSpace(:,2) == -1),3);
            else
                P(cell,cellArrWithCont,controls(i)) = P(cell,cellArrWithCont,controls(i)) + disturbanceSpace(find(disturbanceSpace(:,2) == -1),3);
            end
            %UP
...
function controls = availableControls(cell,wallsMatrix,M, controlSpace)
controls = intersect(find(controlSpace(:,1) == 0), find(controlSpace(:,2) == 0));
%RIGHT CONTROLS
if (wallsMatrix(cell,1) == 0)
    controls = [controls, intersect(find(controlSpace(:,1) == 1),find(controlSpace(:,2) == 0))];
    cellRight = cell + M;
    if(wallsMatrix(cellRight,1) == 0)
        controls = [controls, find(controlSpace(:,1) == 2)];
    end
end
%UP CONTROLS
if (wallsMatrix(cell,2) == 0)
    controls = [controls, intersect(find(controlSpace(:,1) == 0),find(controlSpace(:,2) == 1))];
    cellUp = cell + 1;
    if(wallsMatrix(cellUp,2) == 0)
        controls = [controls, find(controlSpace(:,2) == 2)];
    end
end
%LEFT CONTROLS
...
if (wallsMatrix(cell,1) == 0 && wallsMatrix(cell,2) == 0)
    ...
    if(wallsMatrix(cellRight,2) == 0 && wallsMatrix(cellUp,1) == 0)
        controls = [controls, intersect(find(controlSpace(:,1) == 1),find(controlSpace(:,2) == 1))];
    end
end
